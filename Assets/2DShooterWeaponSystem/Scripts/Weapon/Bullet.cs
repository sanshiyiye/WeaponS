...
public class Bullet : MonoBehaviour {
    ...
    /// </summary>
    public Renderer bulletSpriteRenderer;
    ...
    public virtual float BulletXPosition{
        get{
            return bulletXPosition;
        }
        set{
            bulletXPosition = value;
        }
    }
    ...
    public virtual float BulletYPosition{
        get{
            return bulletYPosition;
        }
        set{
            bulletYPosition = value;
        }
    }

    public virtual void SetBulletPosition(float x, float y)
    {
        bulletXPosition = x;
        bulletYPosition = y;
    }
    ...
    void Update()
    {
        ...
        if (gameObject != null)
        {
            ...
            if (amplitude > 0f)
            {
                ...
                var beta = frequency;//directionAngle + Mathf.PI / 2;
                frequency += frequencyIncrement * (circularFireMode ?  2:1);
                var distance = amplitude;//* Mathf.Sin(frequency);
                var x = bulletXPosition + (circularFireMode ? Mathf.Cos(beta) * distance : 0);
